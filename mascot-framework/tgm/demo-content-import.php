...
class WellcoWPOneClickImport {
	...
	function __construct() {
		...
		//FTP
		$this->revSliderFolder_importURI = 'https://kodesolution.live/one-click-demo-import/wellco/import-rev-sliders/';
		$this->base_url = 'https://kodesolution.live/one-click-demo-import/wellco/v1.0/';

		//HTTP
		$this->preview_base_url = 'https://kodesolution.com/2022/wellco/';
			...
			$final_output[] = array(
					...
					'import_redux'               => array(
							array(
									...
									'option_name' => 'wellco_redux_theme_opt',
							),
					),
					'import_preview_image_url'   => $current_url . 'screenshot.jpg',
					'import_notice'              => esc_html__( 'After you import this demo, you will have to import the slider separately. You will find slider in "import-sliders" folder in your downloaded package.', 'wellco' ),
	...
	public function after_import_setup( $selected_import ) {
		...
		wellco_generate_dynamic_css();
		wellco_generate_css_for_custom_theme_color_from_scss();
	...
	public function plugin_page_setup( $default_settings ) {
		...
		$default_settings['page_title']  = esc_html__( 'One Click Demo Import - ThemeMascot' , 'wellco' );
		$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'wellco' );
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								esc_html_e( 'Error while importing rev sliders', 'wellco' );
								exit;
							}
						}
					}

					esc_html_e( 'Rev sliders imported successfully', 'wellco' );
				} else {
					esc_html_e( 'RevSliderSliderImport instance is empty', 'wellco' );
				}

			} else {
				esc_html_e( 'RevSliderSliderImport class doesn\'t exist', 'wellco' );
			}

		} else {
			esc_html_e( 'Files don\'t exist', 'wellco' );
		}
	}

}

WellcoWPOneClickImport::get_instance();
